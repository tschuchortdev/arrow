...
obs
//ObservableKW(observable=io.reactivex.internal.operators.observable.ObservableFromArray@4d8e3ca8)
```

```kotlin
val flow = Flowable.fromArray(1, 2, 3, 4, 5).k()
flow
//FlowableKW(flowable=io.reactivex.internal.operators.flowable.FlowableFromArray@6f6bf346)
```

```kotlin
val subject = PublishSubject.create<Int>().k()
subject
//ObservableKW(observable=io.reactivex.subjects.PublishSubject@35487173)
```
...
```kotlin
obs.value()
//io.reactivex.internal.operators.observable.ObservableFromArray@4d8e3ca8
```

```kotlin
flow.value()
//io.reactivex.internal.operators.flowable.FlowableFromArray@6f6bf346
```

```kotlin
subject.value()
//io.reactivex.subjects.PublishSubject@35487173
```
